...
**作用**：
- 当回调函数执行时间超过设定的超时时间时，会自动终止该回调任务
- 防止单个回调函数执行时间过长，影响整个系统的响应性能
- 提高系统的可靠性和稳定性

**RAM 占用增加**：
- 任务池：每个任务占用约 256 字节栈空间，默认 4 个任务
...
- **RAM 占用增加**：约 1280 字节
  ...
  - **任务栈**：4 个回调任务 × 256 字节 = 1024 字节（单独计算）
...
- **任务栈占用**：约 1536 字节（基础任务栈 512 字节 + 回调任务 1024 字节）
- **ROM 占用**：约 5.8 KB（仅包含 msg_manager 相关代码）

**说明**：启用 ENABLE_CALLBACK_INTERRUPT 后，ROM 占用会增加约 100-200 字节，RAM 占用基本不变。
...
- v0.1.0：初始版本，支持基本的消息传递功能
- v0.1.1：添加回调超时检测机制, 添加回调中断处理机制, 优化内存管理，添加消息池
